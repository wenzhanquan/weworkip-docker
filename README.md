...
1. **访问 Web 面板**：容器启动后，在浏览器中访问 `http://<你的服务器IP>:8888`（端口取决于你的 compose 映射）。
2. **扫描二维码**：
   - 如果是首次运行，或者检测到 Cookie 已经失效，页面上会显示一个**企业微信登录二维码**。
   - 打开手机企业微信，扫描二维码并确认登录。
3. **完成配置**：扫码完成后，后台会自动接管流程，并将 Cookie 保存到映射的 `./data` 目录中。Web 页面会显示“**正常运行中**”。
4. **静默守护**：此后你无需再进行任何操作。程序会按照你设定的频率在后台默默检查 IP 并自动更新到企业微信后台（并兼具自动保活 Cookie 功能）。
